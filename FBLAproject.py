...
############################
conn = sqlite3.connect("fbla_database.db")
cursor = conn.cursor()
...
#cursor.execute("UPDATE users SET role='admin' WHERE Username = 'Alexey'")
#conn.commit()
...
def partner_list():
  partner_list = []
  cursor.execute("SELECT Name FROM partner_info")
...
def data_window():
  ...
  def view_users():
    ...
    def role_change():
      ...
      for i in selected_users:
        if i[2] == "admin":
          admin_role_change = "UPDATE users SET role='user' WHERE Username = (?)"
          cursor.execute(admin_role_change, (i[0],))
          conn.commit()
        elif i[2] == "user":
          user_role_change = "UPDATE users SET role='admin' WHERE Username = (?)"
  ...
  def update_listbox():
    ...
    cursor.execute("SELECT * FROM partner_info")
  ...
  def add_partner():
    ...
    if new_p_name.get() not in partner_list():
      add_query = "INSERT INTO partner_info VALUES (?,?,?,?,?)"
  ...
  def delete_partner():
    if p_delete.get() in partner_list():
      delete_query = ("DELETE FROM partner_info WHERE Name=(?)")
  ...
  def apply_ps_filter():
    ...
    filter_query = "SELECT * FROM partner_info WHERE product_service = (?)"
  ...
  def apply_purpose_filter():
    ...
    for i in filter_purpose_list:
      ...
      if (filter_purpose_dict[i]).get() == 'True':
        temp_filter_list.append(i)
    filter_query = "SELECT * FROM partner_info WHERE available_resource = (?)"
    for i in temp_filter_list:
      cursor.execute(filter_query, (str(i),))
      temp_filter_result.append(cursor.fetchall())
    listbox.delete(0,END)
